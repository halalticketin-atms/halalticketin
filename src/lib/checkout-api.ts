    ...
    if (!response.ok) {
      ...
      const retryAfterFromBody =
        typeof (data as { retryAfter?: unknown } | null)?.retryAfter === 'number'
          ...
          : typeof (data as { error?: { retryAfter?: unknown } } | null)?.error?.retryAfter ===
              'number'
      ...
      const adjustedItems = Array.isArray(details.adjustedItems)
        ? details.adjustedItems.filter(
            (item): item is { ticketTypeId: string; quantity: number } =>
              Boolean(item) &&
...
export async function createCheckoutSession(
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/api/v1/events/${eventId}/checkout/session`, {
      method: 'POST',
      headers,
      body: JSON.stringify(request),
    });
...
export async function validatePromoCode(
  ...
  try {
    ...
    const payload: { promoCode: string; items: CartItem[]; subtotal?: number } = {
      promoCode,
      items,
    };
    ...
    const response = await fetch(`${API_URL}/api/v1/events/${eventId}/checkout/validate-promo`, {
      method: 'POST',
      headers,
...
> {
  try {
    ...
    const response = await fetch(`${API_URL}/api/v1/public/events/${eventSlug}/unlocked-tickets`, {
      method: 'POST',
      headers,
