...
export default function AnalyticsPage() {
  ...
  const selectedEventMeta =
    selectedEvent === 'all' ? null : eventOptions.find((event) => event.id === selectedEvent);
    ...
    return [
      {
        ...
        subtitle: buildRevenueSplitLabel(
          analytics.stats.ticketRevenue ??
            Math.max(0, analytics.stats.netRevenue - analytics.stats.donationRevenue),
          analytics.stats.donationRevenue,
          analytics.stats.currency
        ),
        icon: DollarSign,
        cardClass:
          'bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-950/30 dark:to-cyan-950/30 border-blue-100 dark:border-blue-900',
      ...
      },
      {
        ...
        icon: Ticket,
        cardClass:
          'bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-950/30 dark:to-purple-950/30 border-indigo-100 dark:border-indigo-900',
  ...
  const derivedMetrics = useMemo(() => {
    ...
    const avgTicketPrice =
      analytics.stats.ticketsSold > 0
        ? (analytics.stats.ticketRevenue ??
            Math.max(0, analytics.stats.netRevenue - analytics.stats.donationRevenue)) /
          analytics.stats.ticketsSold
        : 0;

    const avgOrderValue =
      analytics.stats.paidOrders > 0 ? analytics.stats.netRevenue / analytics.stats.paidOrders : 0;

    const peakMonth = analytics.charts.revenueMonthly.reduce(
    ...
    const growth =
      months.length >= 2
        ? {
            percentage:
              months[months.length - 2].value > 0
                ? ((months[months.length - 1].value - months[months.length - 2].value) /
                    months[months.length - 2].value) *
                  100
                : 0,
  ...
  const currentSeries = useMemo(() => {
    ...
    const sourceData =
      chartView === 'revenue' ? analytics.charts.revenueMonthly : analytics.charts.ticketsMonthly;
  ...
  const topEvents = useMemo(() => {
    ...
    return [...analytics.eventPerformance]
      .sort((a, b) =>
        eventSortBy === 'revenue' ? b.revenue - a.revenue : b.ticketsSold - a.ticketsSold
      )
      .slice(0, 5);
  ...
  const chartPoints = useMemo(() => {
    ...
    return currentSeries.map((point, i) => ({
      x: currentSeries.length === 1 ? plotWidth / 2 : (i / (currentSeries.length - 1)) * plotWidth,
              ...
              <Card className={stat.cardClass}>
                <CardContent className="p-4 sm:p-6">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div
                      className={`flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl ${stat.iconClass}`}
                    >
              ...
              <Card className="border-border/50 gap-4 overflow-hidden sm:gap-6">
                ...
                <CardContent className="px-4 sm:px-6">
                  ...
                  <div
                    ...
                    className="relative h-[240px] sm:h-[280px] min-w-0 overflow-hidden"
                  >
                    <svg
                  ...
                  {topEvents.length === 0 ? (
                    ...
                    <div className="space-y-5">
                      {topEvents.map((event, i) => {
                        const currentValue =
                          eventSortBy === 'revenue' ? event.revenue : event.ticketsSold;
                        const percentage = (currentValue / maxEventValue) * 100;
                        const ticketRevenue =
                          event.ticketRevenue ??
                          Math.max(0, event.revenue - (event.donationRevenue ?? 0));
                            ...
                            <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 pl-7 sm:flex sm:gap-3 sm:pl-8">
                              <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                                <motion.div
                                  ...
                                  style={{
                                    backgroundColor:
                                      eventSortBy === 'revenue' ? '#10b981' : '#3b82f6',
                                  }}
                                  initial={{ width: 0 }}
            ...
            >
              <Card className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 border-green-100 dark:border-green-900">
                <CardContent className="p-4 sm:p-6">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Based on ticket revenue only</p>
                </CardContent>
              </Card>
            </motion.div>
                  ...
                  {derivedMetrics.peakMonth ? (
                    <>
                      <p className="text-2xl sm:text-3xl font-bold">
                        {derivedMetrics.peakMonth.label}
                      </p>
                      <p className="text-sm text-muted-foreground mt-2 break-words">
                        {formatCurrency(
                          derivedMetrics.peakMonth.value,
                          analytics?.stats.currency ?? 'GBP'
                        )}
                      </p>
                    </>
                  ...
                  {derivedMetrics.growth ? (
                    <>
                      <div className="flex items-center gap-2">
                        <p
                          className={`text-2xl sm:text-3xl font-bold ${derivedMetrics.growth.isPositive ? 'text-emerald-500' : 'text-red-500'}`}
                        >
                          {derivedMetrics.growth.isPositive ? '+' : ''}
                          {derivedMetrics.growth.percentage.toFixed(1)}%
                        </p>
                        {derivedMetrics.growth.isPositive ? (
