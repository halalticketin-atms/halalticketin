...
function formatEventRevenue(amount: number, currency: string): string {
  try {
    return new Intl.NumberFormat('en-GB', { style: 'currency', currency }).format(amount);
  } catch {
    const symbol =
      SUPPORTED_CURRENCIES[currency as keyof typeof SUPPORTED_CURRENCIES]?.symbol || currency;
            ...
            <div className="h-1 bg-background rounded-full overflow-hidden shadow-inner">
              <div
                ...
                style={{
                  width:
                    totalTickets > 0
                      ? `${Math.min((ticketsSold / totalTickets) * 100, 100)}%`
                      : '0%',
                }}
              />
            </div>
...
export default function MyEventsPage() {
  ...
  const getFilteredEvents = (status: string) => {
    ...
    if (status === 'cancelled')
      return visibleEvents.filter((event) => event.status === 'cancelled');
    if (status === 'past') {
  ...
  const handleBulkDeleteConfirm = async () => {
    ...
    setBulkDeleteError(null);
    try {
      await Promise.all(draftSelected.map((event) => api.delete(`/api/v1/events/${event.id}`)));
  ...
  const handleBulkArchiveConfirm = async () => {
    ...
    setBulkArchiveError(null);
    try {
      await Promise.all(archivableSelected.map((event) => archiveEvent(event.id)));
            ...
            <TabsContent key={tab} value={tab}>
              {(() => {
                ...
                const allSelectedInTab =
                  tabEventIds.length > 0 && selectedInTab.length === tabEventIds.length;
                  ...
                  return (
                    <Card className="p-12 text-center">
                      ...
                      <Button asChild className="mt-4">
                        <Link
                          href={
                            organizerId ? `/events/new?organizerId=${organizerId}` : '/events/new'
                          }
                        >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                      {tabEvents.map((event, i) => (
                        <EventCard
                          ...
                          onUnarchive={(selectedEvent) => handleUnarchive(selectedEvent.id)}
            ...
            <DialogDescription asChild>
              <div className="text-sm text-muted-foreground space-y-3 pt-2">
                <p>
                  ...
                  {archivableSelected.length === 1 ? '' : 's'}. Archived events are hidden from
                  public listings and checkout.
                </p>
                {hasActiveSelected && (
                  <p className="text-xs text-muted-foreground">
